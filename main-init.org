...
** Completion
   ...
   This section isn't entirely about completion and there are
   some other packages that depend on this setup.

   If you picture doing something like ="C-x C-f"= (~find-file~) then this section
   configures how the list of choices are made, displayed, and actions taken.  The
   packages to do this are:

   - =vertico= displays the completions in a vertical list in the mini-buffer
     or a grid in the mini-buffer or a long list in a new buffer.
   - =orderless= controls how the items are completed.  For example
     matching a substring or any word.
   - =consult= isn't really about completion but instead changes
     many built in commands so while doing completion it previews
     what would happen.  For example, when doing "=C-x b=" the
     buffer of the current selection in the completions is displayed.
   - =embark= adds a new interaction mode to emacs.  It changes emacs
     from mode of (1) choose command; (2) choose what it operates on
     to an additional mode that instead (1) choose what to operate on;
     (2) choose command.
...
*** orderless
    ...
    #+end_src
*** consult
    This configures many built-in commands give a preview of the current
    item in the selection list.  For example, when choosing a buffer the
    current selections buffer is displayed.

    - =embark-consult= integrates =embark= with =consult=
    - =consult-ag= integrates =silver searcher= (=ag=) with =consult=

    See [[https://github.com/minad/consult][consult]] for more details
...
** Programming Languages
   Setting up Language Server Protocol (LSP) is rather involved.

   - =copilot= Is not approved for use.
   - =company=
*** treesit-auto
...
Here's my history of using themes.

| Name                 |    Started |      Ended |
|----------------------+------------+------------|
| modus-vivendi        | 2025-02-14 |            |
| deeper-blue          | 2024-11-19 | 2025-02-14 |
| timu-caribbean-theme | 2024-10-31 | 2024-11-19 |
| nord                 |            | 2024-10-31 |
-
